                        ...
                        <div class="mt-5">
                            ...
                            <form wire:submit.prevent="save">
                                <div class="grid gap-y-4">
                                    @if(session('success'))
                                    <div class="mt-2 mb-4 bg-teal-100 border border-teal-200 text-sm text-teal-800 rounded-lg p-4 dark:bg-teal-800/10 dark:border-teal-900 dark:text-teal-500" role="alert">
                                        {{session('success')}}
                                    </div>
                                    @endif
                                    @if(session('error'))
                                    ...
                                    <!-- End Form Group -->
                                    <button type="submit"
                                        class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"><span wire:loading.remove wire:target='save'>Reset Password</span><span wire:loading wire:target='save'><div class="border-gray-300 h-4 w-4 animate-spin rounded-full border-2 border-t-blue-600" /></span></button>
                                </div>
                            </form>
                            <!-- End Form -->
                        </div>
