<div>
    <div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
        ...
        <div class="flex flex-col bg-white p-5 rounded mt-4 shadow-lg">
                            ...
                            <tbody>
                                ...
                                <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key="{{ $order->id }}">
                                    ...
                                    if ($order->status == 'processing') {
                                        $status = '<span class="bg-amber-500 py-1 px-3 rounded text-white shadow">Processing</span>';
                                    }
                                ...
                                @php
                                    if ($order->payment_status == 'paid') {
                                        $payment_status ='<span class="border-green-500 border-2 text-green-500 py-1 px-3  rounded-md shadow">Paid</span>';
                                    }
                                    if ($order->payment_status == 'pending') {
                                       $payment_status ='<span class="border-blue-500 border-2 text-blue-500 py-1 px-3  rounded-md shadow">Pending</span>';
                                    }
                                    if ($order->payment_status == 'failed') {
                                       $payment_status ='<span class="border-red-500 border-2 text-red-500 py-1 px-3  rounded-md shadow">Failed</span>';
                                    }
                                @endphp
                                    <td
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <a href="/my-order/{{ $order->id }}"
                                            class="bg-slate-600 text-white py-2 px-4 rounded-md hover:bg-slate-500" wire:navigate>View
                                            Details</a>
                                    </td>
                                </tr>
                                @endforeach
           ...
           <div class="mt-6 mx-6"> {{$orders->links()}}</div>
        </div>
    </div>
</div>